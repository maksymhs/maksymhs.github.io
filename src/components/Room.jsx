...
function PixelDonkey({ position, facing = 0 }) {
  const ref = useRef()
  const headRef = useRef()
  const tailRef = useRef()
  const flRef = useRef()
...
function PixelChicken({ position, color = '#f0e8d0', facing = 0, playerRef, catRef }) {
  const ref = useRef()
  const headRef = useRef()
  ...
  useFrame((state) => {
    ...
    if (cooldown.current > 0) {
      cooldown.current -= dt
      // Frozen during cooldown — only lerp rotation, no movement
    ...
    const pickFleeAngle = (cx, cz) => {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        // Score = distance from threat at that point
        const dx = testX - threatPx, dz = testZ - threatPz
    ...
    // Flee if threat is close — only set direction once on enter
    ...
    if (fleeing.current) {
      ...
      // If next step would hit a wall, recalculate direction
      if (isBlocked(nx, nz)) {
...
function Outdoor({ view, playerRef, catRef, onNpcNear, npcInteractRef }) {
  ...
  return (
    <group>
      ...
      {/* Donkey near the vegetable garden corner */}
      <PixelDonkey position={[-28, 0, -28]} facing={Math.PI * 0.3} />

      {/* Chickens near the vegetable garden entrance */}
